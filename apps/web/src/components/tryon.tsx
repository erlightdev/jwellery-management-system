...
const DEFAULT_ADJ: Adj = { x: 0, y: 0, scale: 1, opacity: 0.95 };
...
function overlayJewel(
	...
	// ── Face-based (earring / necklace) ──────────────────────────────────
	if ((type === "earring" || type === "necklace") && faceLms && faceLms.length >= 468) {
		const [elx, ely] = toPx(faceLms[234], cw, ch, mirror); // left ear canal
		const [erx, ery] = toPx(faceLms[454], cw, ch, mirror); // right ear canal
		const [, chiny]  = toPx(faceLms[152], cw, ch, mirror); // chin
		const [, topy]   = toPx(faceLms[10],  cw, ch, mirror); // forehead
		...
		if (type === "earring") {
			...
			drawJewel(ctx, img, elx, ely + drop, sz, adj);
			drawJewel(ctx, img, erx, ery + drop, sz, adj);
			return true;
		}
		if (type === "necklace") {
			drawJewel(ctx, img, cx, chiny + faceH * 0.28, faceW * 1.1, adj);
			return true;
		}
	}

	// ── Hand-based (ring / bracelet) ─────────────────────────────────────
	if ((type === "ring" || type === "bracelet") && handLms && handLms.length > 0) {
		...
		if (type === "ring") {
			...
			drawJewel(ctx, img, (r13x + r14x) / 2, (r13y + r14y) / 2, fingerLen * 0.75, adj, angle);
			return true;
		}
		if (type === "bracelet") {
			...
			const wristW = Math.hypot(p17x - i5x, p17y - i5y) * 1.3;
			const angle  = Math.atan2(p17y - i5y, p17x - i5x);
			drawJewel(ctx, img, wx, wy, wristW, adj, angle);
			return true;
		}
	}
...
export function JewelleryTryon() {
	...
	const touchOrigin = useRef<{ x: number; y: number; ax: number; ay: number; dist?: number } | null>(null);
	...
	useEffect(() => {
		...
		(async () => {
			try {
				...
				const makeOpts = (modelAssetPath: string, extra: object) => ({
					baseOptions: { modelAssetPath, delegate: "GPU" as const },
					...extra,
				});

				const [face, hand] = await Promise.all([
					FaceLandmarker.createFromOptions(vision, makeOpts(FACE_TASK, {
						runningMode: "VIDEO" as const, numFaces: 1,
					})).catch(() => FaceLandmarker.createFromOptions(vision, makeOpts(FACE_TASK, {
						baseOptions: { modelAssetPath: FACE_TASK, delegate: "CPU" as const },
						runningMode: "VIDEO" as const, numFaces: 1,
					}))),
					HandLandmarker.createFromOptions(vision, makeOpts(HAND_TASK, {
						runningMode: "VIDEO" as const, numHands: 2,
					})).catch(() => HandLandmarker.createFromOptions(vision, makeOpts(HAND_TASK, {
						baseOptions: { modelAssetPath: HAND_TASK, delegate: "CPU" as const },
						runningMode: "VIDEO" as const, numHands: 2,
					}))),
				]);
	...
	const tick = useCallback(() => {
		...
		if (faceLMRef.current && (jt === "earring" || jt === "necklace")) {
			try {
				const r = faceLMRef.current.detectForVideo(video, now);
				faceLmsRef.current = r.faceLandmarks?.[0] ?? null;
				setDetected(!!faceLmsRef.current);
			} catch { /* gpu busy, skip frame */ }
		} else {
			faceLmsRef.current = null;
		}

		if (handLMRef.current && (jt === "ring" || jt === "bracelet")) {
			try {
				const r = handLMRef.current.detectForVideo(video, now);
				handLmsRef.current = r.landmarks?.length ? r.landmarks : null;
				setDetected(!!handLmsRef.current);
			} catch { /* gpu busy, skip frame */ }
		} else {
			handLmsRef.current = null;
		}

		if (jewImgRef.current) {
			overlayJewel(ctx, jewImgRef.current, jt,
				faceLmsRef.current, handLmsRef.current,
				vw, vh, adjRef.current, true);
		}
	...
	// ── Photo detection + render ────────────────────────────────────────────
	const renderPhoto = useCallback(() => {
		const canvas = canvasRef.current;
		const bodyImg = bodyImgRef.current;
		if (!canvas || !bodyImg) return;

		// Scale large photos down to 1920px max for performance
		const MAX = 1920;
		...
		if (jewImgRef.current) {
			overlayJewel(ctx, jewImgRef.current, jtypeRef.current,
				faceLmsRef.current, handLmsRef.current,
				canvas.width, canvas.height, adjRef.current, false);
		}
	}, []);

	const detectPhoto = useCallback(async () => {
		if (isDetecting.current || !bodyImgRef.current) return;
		if (!faceLMRef.current || !handLMRef.current) {
			renderPhoto();
			return;
		}
	...
	// Re-render on adj change (photo mode)
	useEffect(() => {
		if (mode === "photo" && bodyImgRef.current) renderPhoto();
	}, [adj, mode, renderPhoto]);
	...
	// Detect again when jewellery type changes (photo mode)
	useEffect(() => {
		setAdj(DEFAULT_ADJ);
		setDetected(false);
	...
	// ── Jewellery image upload + bg removal ─────────────────────────────────
	const handleJewelleryFile = useCallback(async (file: File) => {
		...
		try {
			...
			const blob = await removeBackground(file, { publicPath: IMGLY_CDN });
	...
	const onMouseMove = (e: React.MouseEvent) => {
		...
		setAdj(a => ({
			...a,
			x: dragOrigin.current.ax + (e.clientX - dragOrigin.current.mx),
			y: dragOrigin.current.ay + (e.clientY - dragOrigin.current.my),
		}));
	};
	...
	const onTouchStart = (e: React.TouchEvent) => {
		if (e.touches.length === 1) {
			const t = e.touches[0];
			touchOrigin.current = { x: t.clientX, y: t.clientY, ax: adjRef.current.x, ay: adjRef.current.y };
		} else if (e.touches.length === 2) {
			const dx = e.touches[0].clientX - e.touches[1].clientX;
			const dy = e.touches[0].clientY - e.touches[1].clientY;
			touchOrigin.current = { x: 0, y: 0, ax: adjRef.current.x, ay: adjRef.current.y, dist: Math.hypot(dx, dy) };
		}
	};
	const onTouchMove = (e: React.TouchEvent) => {
		...
		if (e.touches.length === 1) {
			setAdj(a => ({
				...a,
				x: touchOrigin.current!.ax + (e.touches[0].clientX - touchOrigin.current!.x),
				y: touchOrigin.current!.ay + (e.touches[0].clientY - touchOrigin.current!.y),
			}));
		} else if (e.touches.length === 2 && touchOrigin.current.dist) {
			const dx   = e.touches[0].clientX - e.touches[1].clientX;
			const dy   = e.touches[0].clientY - e.touches[1].clientY;
			const dist = Math.hypot(dx, dy);
			setAdj(a => ({ ...a, scale: Math.max(0.1, Math.min(5, a.scale * (dist / touchOrigin.current!.dist!))) }));
			touchOrigin.current = { ...touchOrigin.current, dist };
		}
	};
	...
	const download = () => {
		...
		const a = Object.assign(document.createElement("a"), {
			href: captured, download: `jewel-tryon-${Date.now()}.png`,
		});
		a.click();
	};
	...
	return (
		<div className="flex h-full flex-col gap-6 lg:flex-row">
			...
			<div className="flex w-full flex-col gap-5 lg:w-64 lg:shrink-0">
				...
				<div>
					<p className="mb-2 text-[10px] font-semibold uppercase tracking-widest text-[#888882]">
						Jewellery Image
					</p>
					<label className={`relative flex min-h-32 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed transition-colors ${
						removing
							...
							: jewUrl
							? "border-[#C4A84F]/50 bg-[#FAFAF8] dark:bg-[#0D0D0B]"
						...
						{removing ? (
							...
							<>
								...
								<span className="text-[10px] text-[#C4A84F]">✓ Ready</span>
								<span className="text-[10px] text-[#ADADAA]">Click to change</span>
							</>
						) : (
							<>
								<ImagePlus className="h-6 w-6 text-[#ADADAA]" />
								<span className="text-center text-[11px] text-[#888882]">Drop image or click to upload</span>
				...
				<div>
					...
					<div className="flex overflow-hidden rounded-lg border border-[#E5E5E0] dark:border-[#2A2A28]">
						{(["live", "photo"] as Mode[]).map(m => (
							...
							>
								{m === "live"
									? <><Camera className="h-3.5 w-3.5" /> Live Webcam</>
				...
				{mode === "photo" && (
					<div>
						...
						<label className={`flex min-h-24 cursor-pointer flex-col items-center justify-center gap-2 rounded-xl border-2 border-dashed transition-colors ${
							photoUrl
								? "border-[#6B6B67]/40 bg-[#FAFAF8] dark:bg-[#0D0D0B]"
							...
							{detecting ? (
								<>
									<div className="h-4 w-4 animate-spin rounded-full border-2 border-[#C4A84F] border-t-transparent" />
									<span className="text-[11px] text-[#888882]">Detecting…</span>
								</>
							) : photoUrl ? (
								<>
								...
								<>
									<Upload className="h-5 w-5 text-[#ADADAA]" />
									<span className="text-[11px] text-[#888882]">Upload your photo</span>
									<span className="text-[10px] text-[#ADADAA]">Face, hands, or body</span>
								</>
							)}
						</label>
					</div>
				...
				{/* Status / hints */}
				{canvasLive && jewUrl && (
					<p className="text-[10px] text-[#ADADAA]">Drag to reposition · Scroll / pinch to resize</p>
				)}
				{camErr && <p className="text-[10px] text-red-500">{camErr}</p>}
				{mpErr  && <p className="text-[10px] text-amber-500">AI unavailable: {mpErr}</p>}
			</div>
			...
			<div className="flex flex-1 flex-col gap-3">
				...
				>
					{/* Hidden video element (feeds live canvas) */}
							...
							{mode === "live" ? (
								<>
									<div className="flex h-20 w-20 items-center justify-center rounded-full border border-[#2A2A28]">
										<ScanFace className="h-9 w-9 text-[#888882]" />
									</div>
									<div>
										<p className="text-sm font-medium text-[#F0F0EB]">
											{!mpReady && !mpErr ? "Loading AI vision…" : "Starting camera…"}
										</p>
								...
								<>
									<div className="flex h-20 w-20 items-center justify-center rounded-full border border-[#2A2A28]">
										<Upload className="h-9 w-9 text-[#888882]" />
									</div>
									<div>
										<p className="text-sm font-medium text-[#F0F0EB]">Upload your photo</p>
										<p className="mt-1 text-[11px] text-[#888882]">Supports face, hand, and full body shots</p>
									</div>
								</>
							)}
						</div>
					...
					<div className="pointer-events-none absolute right-3 top-3 flex flex-col items-end gap-2">
						{!mpReady && !mpErr && (
							<div className="flex items-center gap-1.5 rounded-full bg-black/60 px-3 py-1 text-[10px] text-white backdrop-blur-sm">
								<div className="h-1.5 w-1.5 animate-pulse rounded-full bg-[#C4A84F]" />
								Loading AI…
							</div>
						)}
				...
				{captured && (
					<div className="flex items-center gap-3 rounded-xl border border-[#E5E5E0] bg-white p-3 dark:border-[#2A2A28] dark:bg-[#141412]">
						...
						<button
							onClick={download}
							className="flex shrink-0 items-center gap-1.5 rounded-lg bg-[#C4A84F] px-4 py-2 text-[11px] font-medium text-white"
						>
